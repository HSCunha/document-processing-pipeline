...
def preview_route():
    ...
    temp_file = None # Initialize temp_file here for access in finally
    try:
        parts = path.split('/', 1)
        if len(parts) < 2:
        ...
        try: # Inner try for temp_file download and processing
            temp_file = azure_explorer.download_blob(container_name, blob_name)
            
            # Check if temp_file was successfully created
    ...
    except Exception as e: # Outer except to catch any other errors in the route
